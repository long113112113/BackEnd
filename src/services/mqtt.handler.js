    ...
    if (topic === mqttConfig.TOPICS.SCAN) {
        ...
        try {
            ...
            try {
                ...
                logger.info(`[MQTT] AES-GCM decrypt failed device_id=${device_id}`);
            ...
            try {
                ...
                logger.info(`[MQTT] Invalid inner JSON from device_id=${device_id}`);
            ...
            if (receivedHmac.length !== 64 || !/^[0-9a-fA-F]+$/.test(receivedHmac)) {
                logger.info(`[MQTT] Invalid HMAC format device_id=${device_id} len=${receivedHmac.length}`);
            ...
            if (!crypto.timingSafeEqual(hash1, hash2)) {
                logger.info(`[MQTT] HMAC mismatch device_id=${device_id}`);
                const errPayload = encryptError(aesKey, device_id, 'error', 'HMAC verification failed', { card_uid });
                mqttConfig.publish(`${mqttConfig.TOPICS.RESULT}/${device_id}`, errPayload);
                return;
            }

            const nonceCheck = await verifyNonce(nonce);
            if (!nonceCheck.ok) {
                logger.info(`[MQTT] Nonce check failed device_id=${device_id}: ${nonceCheck.reason}`);
            ...
            if (!seqCheck.ok) {
                logger.info(`[MQTT] Seq check failed device_id=${device_id}: ${seqCheck.reason}`);
            ...
            logger.error('[MQTT] Attendance processing failed:', err.message);
            if (device_id) {
